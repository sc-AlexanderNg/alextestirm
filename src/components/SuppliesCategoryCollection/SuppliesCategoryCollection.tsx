...
import styles from './SuppliesCategoryCollection.module.css';
...
const SuppliesCategoryCollection = (props: SearchResultsProps): JSX.Element => {
  ...
  const facettypefilter = `im_category:'${props?.params[
    'Facet Type'
  ]?.replaceAll('-', ' ')}'`;
  ...
  return isEditing ? (
    ...
    <div data-is-resource-header="true">
      ...
      <InstantSearch
        ...
        routing={routing}
      >
        <Configure filters={facettypefilter} />
...
export default withDatasourceCheck()<SearchResultsProps>(
  SuppliesCategoryCollection
);
