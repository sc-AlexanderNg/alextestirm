...
import { getDictionaryLocale } from 'src/helpers/DictionaryLocaleHelper';
...
class NormalModePlugin implements Plugin {
  ...
  async exec(
    ...
    if (process.env.NEXT_PUBLIC_ENABLE_LOCALE_FORMAT_DICTIONARY === 'true') {
      locale = getDictionaryLocale(props.locale);
